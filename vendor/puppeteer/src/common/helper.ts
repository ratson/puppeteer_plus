...
import type { Readable } from 'https://deno.land/std@0.135.0/node/stream.ts';
...
async function getReadableAsBuffer(
  ...
  let fileHandle: import('https://deno.land/std@0.135.0/node/fs.ts').promises.FileHandle | undefined;
...
async function getReadableFromProtocolStream(
  ...
  const { Readable } = await import('https://deno.land/std@0.135.0/node/stream.ts');
 ...
 * `(await import('https://deno.land/std@0.135.0/node/fs.ts')).promises`.
 */
async function importFSModule(): Promise<typeof import('https://deno.land/std@0.135.0/node/fs.ts')> {
  if (!isNode) {
    throw new Error('Cannot load the fs module API outside of Node.');
  }

  const fs = await import('https://deno.land/std@0.135.0/node/fs.ts');
  if (fs.promises) {
    return fs;
  }
  return fs;
